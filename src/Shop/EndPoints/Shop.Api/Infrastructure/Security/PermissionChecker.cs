...
public class PermissionChecker : AuthorizeAttribute, IAsyncAuthorizationFilter
{
    ...
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        {
            if (await UserHasPermission(context) is false)
            {
    ...
    private async Task<bool> UserHasPermission(AuthorizationFilterContext context)
    {
        ...
        var userRoles = roles.Where(r => roleIds.Contains(r.Id)).ToList();

        return userRoles.Any(r => r.Permissions != null && r.Permissions.Contains(_permission));
    }
}