...
internal class RoleFacade : IRoleFacade
{
    ...
    public async Task<OperationResult> DeleteRole(long roleId, CancellationToken cancellationToken = default)
    {
        return await _mediator.Send(new DeleteRoleCommand(roleId), cancellationToken);
    }
